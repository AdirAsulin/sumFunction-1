...
const doors = [2, 4, 5];
const DOM = {};
const searchBtn = document.getElementById("searchCar")
...
function getListItem(carData) {
  ...
  listItem.innerText = `car model:${carData.type}, licence Plate: ${carData.lp}, color: ${carData.color}, sunroof: ${carData.isSunRoof}, doors: ${carData.doors}`;
  return listItem;
}
...
function getCardItem(carData) {
  ...
  mainDivCard.classList = `card text-white bg-${getClassByColor(
    carData.color
  )} mb-3 col-4`;
...
function carFinder() {
  const carsInput = document.getElementById("searchCarInput");
  const filter = carsInput.value.toUpperCase();
  const ul = document.getElementById("data");
  const li = ul.getElementsByTagName("li");

  for (let index = 0; index < li.length; index++) {
    let liValue = ul.getElementsByTagName("li")[index];
